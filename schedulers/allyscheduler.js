...
function login() {
    ...
    var post_data = 'userNamePvtEncrypt='+process.argv[2]+'&rememberMeFlag=false&passwordPvtBlock=' + process.argv[3];
...
function getData(res) {
    ...
    res.on("data", function(chunk) {
        ...
        var post_options = {
            host: 'securebanking.ally.com',
            port: '443',
            path: '/IDPProxy/executor/accounts/' + accountId + '/transactions.csv?patron-id=olbWeb&fromDate='+year+'-01-01&toDate='+year+'-12-31&status=Posted',
            method: 'GET',
            headers: {
                'Cookie': sessioncookie,
